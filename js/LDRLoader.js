...
THREE.LDRLoader.prototype.load = function (id) {
    ...
    let onFileLoaded = function (text) {
        if (text.startsWith('{"')) //SD
        {
            ...
            const partNames = Object.keys(text);
            partNames.forEach((id) => {
                let content = text[id];
                content = atob(content.substring('data:text/plain;base64,'.length));
                self.parse(content, id);
            });
    ...
    for (let i = 0; i < dataLines.length; i++) {
        ...
        switch (lineType) {
            case 0:
                ...
                else if (is('!COLOUR')) //SD
                {
                    //e.g.: [ "0", "!COLOUR", "Black", "CODE", "0", "VALUE", "#1B2A34", "EDGE", "#808080" ]
                    //TODO: handle new colour definition
                }
                else if (parts[1][0] === "!") {
                    if (is("!THEME") ||
                        is("!HELP") ||
                        ...
                        is("!CATEGORY"))
                    {
                        ...
                        /*
                        //TODO: handle flex parts
                        0 !LEOCAD SYNTH BEGIN
                        0 !LEOCAD SYNTH CONTROL_POINT -100 0 0 1 0 0 0 1 0 0 0 1 12
                        0 !LEOCAD SYNTH CONTROL_POINT 36 120 0 0 -1 0 -1 -0 0 -0 -0 -1 12
                        1 7 260 -254 0 0 0 1 -1 0 0 0 -1 0 55709.dat
                        0 !LEOCAD SYNTH END
                        */
                    }
                    else {
